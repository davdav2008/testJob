...
<form class="form-horizontal" id="create_car" method="post" action="{{route('car.create')}}">
        ...
        <div class="form-group">
            ...
            <div class="col-md-4">
                <select id="Model" name="Model" class="form-control">
                    ...
                    @foreach(\App\CarModel::all() as $model)
                        <option id="{{ DB::table('brands')->whereId($model->brand_id)->get()->pluck('brand')[0]}}">{{$model->name}}</option>
                    @endforeach
                </select>
            </div>
        </div>
    ...
    {{ csrf_field() }}
    <a href="{{route('car.index')}}" class="btn btn-default col-md-offset-3" style="margin-left: 28%;">К списку</a>    <button type="submit"  class="btn btn-default col-md-offset-4" style="    margin-left: 27.7%;">Сохранить</button>
</form>
